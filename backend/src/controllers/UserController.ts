...
import  jwt from 'jsonwebtoken';
  ...
  export const login = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!user) {
        ...
        refferedBy ? user = await UserService.createUser({walletAddress, refferedBy }, signature) :  user = await UserService.createUser({walletAddress}, signature);
      }
      if (!user) {
        res.status(400).json({ 
          success: false, 
  ...
  export const getUserByWallet = async (req: Request, res: Response): Promise<void> => {
    try {
  ...
  export const updatedUserScore = async (req: Request, res: Response): Promise<void> => {
    try {
